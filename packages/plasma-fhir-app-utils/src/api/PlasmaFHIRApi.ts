...
import { PlasmaFHIRUtils } from "./PlasmaFHIRUtils";
...
export class PlasmaFHIRApi {
    ...
    public async readFamilyMemberHistory(
        ...
        serverUrl = "", 
        authToken = ""):
    Promise<PlasmaFHIR.FamilyMemberHistory[]> {

        if (!serverUrl) { serverUrl = this.serverUrl; }
        if (!authToken) { authToken = this.authToken; }
        return this.readPatientResource<PlasmaFHIR.FamilyMemberHistory>(patientId, "FamilyMemberHistory", searchOptions, serverUrl, authToken);
    }
    ...
    public async readAllergyIntolerance(
        ...
        serverUrl = "", 
        authToken = ""):
    Promise<PlasmaFHIR.Observation[]> {

        if (!serverUrl) { serverUrl = this.serverUrl; }
        if (!authToken) { authToken = this.authToken; }
        return this.readPatientResource<PlasmaFHIR.Observation>(patientId, "AllergyIntolerance", searchOptions, serverUrl, authToken);
    }
    ...
    public async readCondition(
        ...
        serverUrl = "", 
        authToken = ""):
    Promise<PlasmaFHIR.Condition[]> {

        if (!serverUrl) { serverUrl = this.serverUrl; }
        if (!authToken) { authToken = this.authToken; }
        return this.readPatientResource<PlasmaFHIR.Condition>(patientId, "Condition", searchOptions, serverUrl, authToken);
    }
    ...
    public async readImmunization(
        ...
        serverUrl = "", 
        authToken = ""):
    Promise<PlasmaFHIR.Immunization[]> {

        if (!serverUrl) { serverUrl = this.serverUrl; }
        if (!authToken) { authToken = this.authToken; }
        return this.readPatientResource<PlasmaFHIR.Immunization>(patientId, "Immunization", searchOptions, serverUrl, authToken);
    }
    ...
    public async readEncounter(
        ...
        serverUrl = "", 
        authToken = ""):
    Promise<PlasmaFHIR.Encounter[]> {

        if (!serverUrl) { serverUrl = this.serverUrl; }
        if (!authToken) { authToken = this.authToken; }
        return this.readPatientResource<PlasmaFHIR.Encounter>(patientId, "Encounter", searchOptions, serverUrl, authToken);
    }
    ...
    Promise<PlasmaFHIR.Observation[]> {
        ...
        if (!serverUrl) { serverUrl = this.serverUrl; }
        if (!authToken) { authToken = this.authToken; }
        return this.readPatientResource<PlasmaFHIR.Observation>(patientId, "Observation", { category: "laboratory" }, serverUrl, authToken);
    }
    ...
    Promise<PlasmaFHIR.Observation[]> {
        ...
        if (!serverUrl) { serverUrl = this.serverUrl; }
        if (!authToken) { authToken = this.authToken; }
        return this.readPatientResource<PlasmaFHIR.Observation>(patientId, "Observation", { category: "vital-signs" }, serverUrl, authToken);
    }
    ...
    Promise<PlasmaFHIR.Observation[]> {
        ...
        if (!serverUrl) { serverUrl = this.serverUrl; }
        if (!authToken) { authToken = this.authToken; }
        return this.readPatientResource<PlasmaFHIR.Observation>(patientId, "Observation", { code: "http://loinc.org|72166-2" }, serverUrl, authToken);
    }
