...
export class PlasmaFHIRUtils {
    // Return the "Conformance Statement" for this FHIR URL...
    public static getConformanceStatementURL(baseURL: string): string {
        if (!baseURL.endsWith("/")) { baseURL += "/"; }
        return `${baseURL}metadata`;        
    }

    // Returns the "Conformance Statement" data as a JSON object...
    public static async getConformanceStatement(baseURL: string): Promise<any> {
    ...
    public static async getConformanceStatementPatientSearchParam(baseURL: string, resourceType: string, cachedConformanceStatementData: any | null = null): Promise<{ searchParam: string | undefined, conformance: any }> {

        // Check if data is cached. If not, retrieve it...
        const conformance = (!!cachedConformanceStatementData) 
            ? cachedConformanceStatementData 
            : await PlasmaFHIRUtils.getConformanceStatement(baseURL);
